...
def _step_plot(path: Path, rows: list[dict], variables: tuple[str, ...], title: str, labels: dict):
    ...
    for axis, variable in zip(axes, variables):
        ...
        if variable in {"p", "pt"} and min(edges) > 0 and max(edges) / min(edges) > 1e4:
            axis.set_xscale("log")
...
def build_part3(samples: list[SampleData], root: Path, second_token: str) -> list[Path]:
    ...
    for data, token in zip(samples, tokens):
        ...
        for method in ASSOCIATIONS:
            integrated = _integrated_rows(data, method)
            ...
            for variable in ("p", "theta"):
                binned = _binned_inefficiency(data, method, variable)
        ...
        for variable in ("p", "theta"):
            combined = [{**row, "association_method": row["association_method"].replace("L_direct", "Ldirect").replace("L_ancestor", "Lancestor")}
                        for method in ASSOCIATIONS for row in _binned_inefficiency(data, method, variable)]
            for row in combined:
    ...
    combined = [row for data in samples for row in _integrated_rows(data, "Lancestor")]
    stem = f"W_vs_{second_token}_Lancestor_reco_efficiency"
    ...
    for variable in ("p", "theta"):
        rows = [row for data in samples for row in _binned_inefficiency(data, "Lancestor", variable)]
        path = out / f"W_vs_{second_token}_Lancestor_reco_inefficiency_vs_{variable}.csv"; write_csv(path, rows); written.append(path)
    ...
    for method in ASSOCIATIONS:
        ...
        stem = f"W_vs_{second_token}_{method}_non_tau_photon_reco_efficiency"
        ...
        for variable in ("p", "theta"):
            ...
            path = directory / f"W_vs_{second_token}_{method}_non_tau_photon_reco_inefficiency_vs_{variable}.csv"
...
def build_part3b(samples: list[SampleData], root: Path, second_token: str) -> list[Path]:
    out = root / "part3b"; out.mkdir(parents=True, exist_ok=True); written = []; suffix = f"W_vs_{second_token}"
    summaries_by_key = {}
    ...
    for data, token in zip(samples, ("W", second_token)):
        rows = []
