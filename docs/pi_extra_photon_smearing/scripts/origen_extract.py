...
for a in t.iterate(BR,step_size=STEP,entry_stop=NMAX):
    ...
    rmass=ak.where(has,pad1(a.RecoTauMass)[rks],-1.); rP=ak.where(has,pad1(a.RecoTauP)[rks],-1.)
    ...
    ph=ak.zip({"P":a.RecoPhotonP,"th":a.RecoPhotonTheta,"phi":a.RecoPhotonPhi,"tk":a.RecoPhotonTauKey,"gmi":gmi,
               "gorig":ak.where(hg,pad1(a.GenPhotonOrigin)[gmis],-1),"gtk":ak.where(hg,pad1(a.GenPhotonTauKey)[gmis],-2),
               "gpar":ak.where(hg,pad1(a.GenPhotonParentPDG)[gmis],0),"gphP":ak.where(hg,pad1(a.GenPhotonP)[gmis],-1.)})
