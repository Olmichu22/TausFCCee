...
for a in uproot.open(F)["Tau_tree"].iterate(BR,step_size=100000,entry_stop=NMAX):
    ...
    rk2=rk[has]
    rtype=pad1(a.RecoTauType)[rks][has]; rdm=pad1(a.RecoTauDM)[rks][has]; rmass=pad1(a.RecoTauMass)[rks][has]; rP=pad1(a.RecoTauP)[rks][has]
    tP=padl(cP)[rks][has]; tTh=padl(cTh)[rks][has]; tPh=padl(cPh)[rks][has]; tPDG=padl(cPDG)[rks][has]
