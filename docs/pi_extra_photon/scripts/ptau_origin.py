...
for a in uproot.iterate(F+":Tau_tree",BR,step_size=100000):
    ...
    orig=ak.where(hg,a.GenPhotonOrigin[gmis],-1); gtk=ak.where(hg,a.GenPhotonTauKey[gmis],-2); gpar=ak.where(hg,a.GenPhotonParentPDG[gmis],0)
    ...
    ph=ak.zip({"P":a.RecoPhotonP,"tk":a.RecoPhotonTauKey,"cat":cat,"gP":ak.where(hg,a.GenPhotonP[gmis],-1.)})
